...
function ContactsSettingsContent() {
  ...
  const { data: googleSyncStatus, refetch: refetchSyncStatus } = useQuery({
  ...
  const contacts: Contact[] = data?.pages.flatMap(page => page.contacts) || []
  ...
  const handleDeleteAll = () => {
    ...
    if (confirm(`Are you sure you want to delete ALL ${totalCount} contacts? This cannot be undone.`)) {
  ...
  const handleDisconnectGoogle = async () => {
    if (!confirm('Are you sure you want to disconnect Google Contacts? This will not delete imported contacts.')) {
      return
    }
    ...
    try {
      ...
      addToast('Google Contacts disconnected', 'success')
      refetchSyncStatus()
    } catch (error) {
      addToast('Failed to disconnect Google', 'error')
    }
  }
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {googleSyncStatus.connected_email && (
                    <p className="text-xs text-gray-500">{googleSyncStatus.connected_email}</p>
                  )}
                  {googleSyncStatus.last_synced && (
                    <p className="text-xs text-gray-500">
                      Last synced: {formatRelativeTime(new Date(googleSyncStatus.last_synced))}
                    </p>
                  )}
                </div>
              </div>
